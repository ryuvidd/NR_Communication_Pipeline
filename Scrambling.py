...
class PDSCHScrambler():
    ...
    def __init__(self, n_RNTI:int, n_ID:int):
        self.n_RNTI = n_RNTI
        self.n_ID = n_ID
    ...
    def process(self, Codeword: np.ndarray) -> list[np.ndarray]:
        ...
        return [Codeword.astype(np.uint8) ^ c]
            ...
            # Codeword.shape is a list of arrays for this case
            # scrambledBits = []
            # for i in range(2):
...
class PDSCHDescrambler():
    ...
    def process(self, ScrambledLLRs: list[np.ndarray]) -> np.ndarray:
        ...
        if len(ScrambledLLRs) != 1:
            raise ValueError(f"Expected 1 codeword.")
...
if __name__ == "__main__":
    ...
    Scrambler = PDSCHScrambler(n_RNTI, n_ID)
    ...
    Descrambler = PDSCHDescrambler(n_RNTI, n_ID)
