...
- A matrix H_BG has shape of 46x68 (for BG1) or 42x52 (for BG2) obtained from the table provided in 5.3.2-2 and 5.3.2-3, loaded using BG_loader (they already did the mod Z_c). 
    - For BG1, H_BG.shape = (46, 68), The num_col = 68 means K_b + num_parity, where K_b = 22 and num_parity = 46.
...
- Final encoded code blcok is fullCodeBlock[2*Z_c:].
...
class LDPCEncoder():
    def __init__(self, config: LDPCConfig):
        ...
        self.K = config.K
        self.K_b = config.K_b
        expected_K = self.K_b * self.Z_c
        if self.K != expected_K:
            raise ValueError(f"Invalid K = {self.K}, expected {expected_K} for BG{self.baseGraph} with Z_c = {self.Z_c}")
    ...
    def encode(self, codeBlocks: list, validityCheckFlag: bool) -> list:
        ...
        for codeBlock in codeBlocks:
            ...
            H_c = self.H_BG[:,:self.K_b]
            H_p = self.H_BG[:,self.K_b:]
            
            # Compute Parities
            c_blocks = NewcodeBlock.reshape(self.K_b, self.Z_c)
...
if __name__ == "__main__":
    ...
    EncodedBlocks = Encoder.encode(codeBlocks, validityCheckFlag=True)